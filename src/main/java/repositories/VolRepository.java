...
public interface VolRepository extends JpaRepository<Vol, Integer>  {
	...
	@Query("select v from Vol v left join fetch v.reservations where v.idVol=:id") //permet de recupérer le vol avec la liste des reservations
	Optional<Vol> findVolWithReservation(@Param("id")Integer idVol); // car le findById standard n'embarque pas les listes dans les objets !
